...
/**
 * 指示書自動生成モジュール v3.0.0 — 分割ボタン対応 + ページ分割方式 + サニタイズ強化
 * 📋ボタン押下時のフロー:
 * 1. 議論内容をテキスト化
...
指示書の品質チェックを行う担当です。安全・正確・実行可能な指示書にしてください。`;
  ...
  async function generate(chatMessages, fileType = 'all') {
    ...
    try {
      ...
      if (allFiles.length === 0) {
        ...
        throw new Error(`ファイルの生成に失敗したよ。もう一回試してみて${detail}`);
      }
  ...
  async function _callOpenAIRaw(systemPrompt, messages) {
    ...
    console.log('[DocGenerator] OpenAI response:', JSON.stringify(data)?.substring(0, 200));
  ...
  async function _callClaudeRaw(systemPrompt, messages) {
    ...
    console.log('[DocGenerator] Claude response:', JSON.stringify(data)?.substring(0, 200));
