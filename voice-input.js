...
class VoiceController {
  ...
  _setupCallbacks() {
    ...
    this._stt.onEnd = () => {
      if (this._voiceState.isSpeaking() || this._playback.isPlaying() || this._playback.isQueuePlaying()) return;
  ...
  init() {
    ...
    console.log(`[Voice] 初期化完了（v2.2） stt=${this._stt.name} cmd=${!!this._voiceCmd}`);
  }
